...
[ -n "${pheno1_id}" ] && [ -n "${pheno2_id}" ] && [ -n "${loci_dir}" ] && [ -n "${ref_prefix}" ] && [ -n "${out_dir}" ] || {
  echo "usage: bin/gen_ld_matrix.sh PHENO1 PHENO2 LOCI_DIR REF_PREFIX OUT_DIR"
  exit 1
}

task_root="$(pwd)"
ref_prefix="${task_root}/${ref_prefix}"
src_loci_dir="${task_root}/${loci_dir}"
dst_loci_dir="${task_root}/${out_dir}"

mkdir -p "${dst_loci_dir}"
cp -R "${src_loci_dir}/." "${dst_loci_dir}/"

find "${dst_loci_dir}" -type d -name 'locus_chr*_*_*' | while read -r locus_dir; do
