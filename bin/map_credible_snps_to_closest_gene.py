...
def load_gencode_pc_gtf(gtf_path: str, chrom: str = None):
    gtf = pd.read_csv(
        gtf_path,
        sep="\t",
        comment="#",
    ...
    if chrom is not None:
        gtf = gtf[gtf["seqname"].eq(str(chrom))].copy()
...
def run_one(input_path: str, gtf_path: str, out_path: str):
    input_path = Path(input_path)
    out_path = Path(out_path)
    name = input_path.name

    m = re.match(r"([A-Za-z0-9]+)_([A-Za-z0-9]+)_(?:defined_loci|lava)_chr(\d+|X|Y|MT)_(\d+)_(\d+)\.tsv$", name)
    if m is None:
        m = re.match(r"([A-Za-z0-9]+)_([A-Za-z0-9]+)_chr(\d+|X|Y|MT)_(\d+)_(\d+)\.tsv$", name)

    if m is None:
        raise ValueError(f"Could not parse locus coordinates from filename: {name}")

    pheno1, pheno2, chrom, lo, hi = m.groups()
    gtf_pc = load_gencode_pc_gtf(gtf_path, chrom=chrom)
...
def main():
    ...
    if args.input is not None and args.out is not None:
        run_one(args.input, args.gtf, args.out)
    else:
