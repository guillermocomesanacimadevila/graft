...
if (length(args) < 20) {
  stop("usage: Rscript ldsc.R pheno1_file pheno2_file pheno1_name pheno2_name case1 ctrl1 case2 ctrl2 pop_prev1 pop_prev2 out_root hm3 ld wld maf_filter info_filter mean_chisq_min intercept_min intercept_max h2_z_min")
}
...
ld_path <- args[13]
wld_path <- args[14]
maf_filter <- as.numeric(args[15])
info_filter <- as.numeric(args[16])
mean_chisq_min <- as.numeric(args[17])
intercept_min <- as.numeric(args[18])
intercept_max <- as.numeric(args[19])
h2_z_min <- as.numeric(args[20])

pair_name <- paste0(pheno1_name, "_", pheno2_name)

N1_eff <- 4 / (1 / case1 + 1 / ctrl1)

if (pheno2_name == "SMOKING") {
  N2_eff <- 462690
  sample_prev <- c(case1 / (case1 + ctrl1), NA)
  pop_prev <- c(pop_prev1, NA)
} else {
  N2_eff <- 4 / (1 / case2 + 1 / ctrl2)
  sample_prev <- c(case1 / (case1 + ctrl1), case2 / (case2 + ctrl2))
...
if (pheno2_name == "SMOKING") {
  ldsc_out <- ldsc(
    traits = out_files,
    sample.prev = c(sample_prev[1], NA),
    population.prev = c(pop_prev[1], NA),
    ld = ld_path,
    wld = wld_path,
    ldsc.log = pair_name
  )
...
if (pheno2_name == "SMOKING") {
  uni_2 <- list(
    S = matrix(c(0.0976), 1, 1),
    V = matrix(c(0.0036^2), 1, 1),
    I = matrix(c(1.0344), 1, 1),
    ldscoutput = data.frame()
  )
} else {
  uni_2 <- ldsc(
    traits = out_files[2],
...
qc_out[, pass_h2z := h2_z > h2_z_min]
qc_out[, pass_mean := ifelse(is.na(mean_chisq), NA, mean_chisq > mean_chisq_min)]
qc_out[, pass_int := Intercept >= intercept_min & Intercept <= intercept_max]
